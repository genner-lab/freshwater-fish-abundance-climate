...
write.csv(dataframe_complete, file="dataframe_complete_03May2024.csv")
...
df_models_max <- dataframe_complete %>%
  ...
  summarise(Slope_value = cov(Avg_Abundance,Maximum.temperature) / var(Avg_Abundance),Year=min(Year),Duration=max(Duration),Latitude=mean(Latitude),Longitude=mean(Longitude),BioRealm=getmode(BioRealm),Elevation=mean(Elevation),Troph=getmode(Troph),Length=mean(Length),Range=mean(range),RelativeLatitude=mean(RelativeLatitude))

#There are 94658 observations at this stage
df_models_max <- subset(df_models_max,!is.na(df_models_max$Slope_value))#remove rows with slope values of NA
#There are 91965 observations at this stage
df_models_max <- subset(df_models_max,!df_models_max$Slope_value == 0)#remove rows with correlation values of 0
#There are 91935 observations at this stage

#make data binomial
df_models_max$AbundanceChange <- ifelse(df_models_max$Slope_value >0, "1","0")

write.csv(df_models_max, file="df_models_max_03May2024.csv")

#Section14--Avg Data:Calculate correlations between climate and abundance-------

df_models_avg <- dataframe_complete %>%
  group_by(TimeSeriesID,Species) %>% #group dataframe by TimeSeriesID & Species
  summarise(Slope_value = cov(Avg_Abundance,Average.temperature) / var(Avg_Abundance),Year=min(Year),Duration=max(Duration),Latitude=mean(Latitude),Longitude=mean(Longitude),BioRealm=getmode(BioRealm),Elevation=mean(Elevation),Troph=getmode(Troph),Length=mean(Length),Range=mean(range),RelativeLatitude=mean(RelativeLatitude))

#There are 94658 observations at this stage
df_models_avg <- subset(df_models_avg,!is.na(df_models_avg$Slope_value))#remove rows with slope values of NA
#There are 91965 observations at this stage
df_models_avg <- subset(df_models_avg,!df_models_avg$Slope_value == 0)#remove rows with correlation values of 0
#There are 91965 observations at this stage

#make data binomial
df_models_avg$AbundanceChange <- ifelse(df_models_avg$Slope_value >0, "1","0")

write.csv(df_models_avg, file="df_models_avg_03May2024.csv")
